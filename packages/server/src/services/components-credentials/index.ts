    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentCredentials, credentialName)) {
            ...
            if (credInstance.icon === undefined) {
                throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Credential ${credentialName} icon not found`)
            }
            ...
            if (credInstance.icon.endsWith('.svg') || credInstance.icon.endsWith('.png') || credInstance.icon.endsWith('.jpg')) {
                ...
                throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Credential ${credentialName} icon is missing icon`)
            }
        } else {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Credential ${credentialName} not found`)
        }
    } catch (error) {
        throw new InternalDtamindError(
            StatusCodes.INTERNAL_SERVER_ERROR,
