...
import { INodeOptionsValue } from 'dtamind-components'
...
const getAllNodes = async () => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: nodesService.getAllNodes - ${getErrorMessage(error)}`)
    }
}
...
const getNodeByName = async (nodeName: string) => {
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentNodes, nodeName)) {
            ...
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Node ${nodeName} not found`)
        }
    } catch (error) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: nodesService.getAllNodes - ${getErrorMessage(error)}`)
    }
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentNodes, nodeName)) {
            ...
            if (nodeInstance.icon === undefined) {
                throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Node ${nodeName} icon not found`)
            }
            ...
            if (nodeInstance.icon.endsWith('.svg') || nodeInstance.icon.endsWith('.png') || nodeInstance.icon.endsWith('.jpg')) {
                ...
                throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Node ${nodeName} icon is missing icon`)
            }
        } else {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Node ${nodeName} not found`)
        }
