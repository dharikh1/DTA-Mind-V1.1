...
const getLogs = async (startDate?: string, endDate?: string) => {
    if (!startDate || !endDate) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: logService.getLogs - No start date or end date provided`)
    }

    if (startDate > endDate) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: logService.getLogs - Start date is greater than end date`)
    }
    ...
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: logService.getLogs - ${getErrorMessage(error)}`)
    }
}
