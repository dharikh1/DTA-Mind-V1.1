...
const deleteLoaderFromDocumentStore = async (
    ...
    try {
        ...
        if (found) {
            ...
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Unable to locate loader in Document Store ${entity.name}`)
        }
    } catch (error) {
        throw new InternalDtamindError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getDocumentStoreFileChunks = async (appDataSource: DataSource, storeId: string, docId: string, pageNo: number = 1) => {
    try {
        ...
        if (!chunksWithCount) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Chunks with docId: ${docId} not found`)
        }
...
const deleteDocumentStore = async (storeId: string, orgId: string, workspaceId: string, usageCacheManager: UsageCacheManager) => {
    try {
        ...
        if (!entity) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Document store ${storeId} not found`)
        }
...
const deleteDocumentStoreFileChunk = async (storeId: string, docId: string, chunkId: string) => {
    try {
        ...
        if (!found) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Document store loader ${docId} not found`)
        }
        ...
        if (!tbdChunk) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Document Chunk ${chunkId} not found`)
        }
...
const deleteVectorStoreFromStore = async (storeId: string) => {
    try {
        ...
        if (!entity.embeddingConfig) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Embedding for Document store ${storeId} not found`)
        }

        if (!entity.vectorStoreConfig) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Vector Store for Document store ${storeId} not found`)
        }
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (input.startsWith('FILE-STORAGE::')) {
            if (!documentStoreEntity) {
                ...
                if (!documentStoreEntity) {
                    throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Document store ${data.storeId} not found`)
                }
            }
...
const queryVectorStore = async (data: ICommonObject) => {
    try {
        ...
        if (!entity) {
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Document store ${data.storeId} not found`)
        }
        const options: ICommonObject = {
            chatflowid: uuidv4(),
            appDataSource: appServer.AppDataSource,
        ...
        if (!entity.embeddingConfig) {
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding for ${data.storeId} is not configured`)
        }

        if (!entity.vectorStoreConfig) {
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector Store for ${data.storeId} is not configured`)
        }
        ...
        if (!retriever) {
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create retriever`)
        }
        const startMillis = Date.now()
        const results = await retriever.invoke(data.query, undefined)
        if (!results) {
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to retrieve results`)
        }
...
const _createEmbeddingsObject = async (
    ...
    if (!embeddingObj) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create EmbeddingObj`)
    }
    return embeddingObj
}
...
const _createRecordManagerObject = async (
    ...
    if (!recordManagerObj) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create RecordManager obj`)
    }
...
const upsertDocStore = async (
    ...
    if (data.metadata) {
        try {
            ...
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, `Error: Invalid metadata`)
        }
    }
    ...
    if (docId) {
        ...
        if (!loader) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Document loader ${docId} not found`)
        }
    ...
    if (!loaderName || !loaderId || !loaderConfig) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Loader not configured`)
    }

    if (!vectorStoreName || !vectorStoreConfig) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector store not configured`)
    }

    if (!embeddingName || !embeddingConfig) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding not configured`)
    }
...
const generateDocStoreToolDesc = async (docStoreId: string, selectedChatModel: ICommonObject): Promise<string> => {
    try {
        ...
        if (!chunks?.length) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `DocumentStore ${docStoreId} chunks not found`)
        }
