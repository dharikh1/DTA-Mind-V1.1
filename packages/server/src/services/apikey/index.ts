...
const getAllApiKeys = async (workspaceId?: string, autoCreateNewKey?: boolean, page: number = -1, limit: number = -1) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.getAllApiKeys - ${getErrorMessage(error)}`)
    }
}
...
const getApiKey = async (apiKey: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.getApiKey - ${getErrorMessage(error)}`)
    }
}
...
const getApiKeyById = async (apiKeyId: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.getApiKeyById - ${getErrorMessage(error)}`)
    }
}
...
const createApiKey = async (keyName: string, workspaceId?: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.createApiKey - ${getErrorMessage(error)}`)
    }
}
...
const updateApiKey = async (id: string, keyName: string, workspaceId?: string) => {
    try {
        ...
        if (!currentKey) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `ApiKey ${currentKey} not found`)
        }
        currentKey.keyName = keyName
        await appServer.AppDataSource.getRepository(ApiKey).save(currentKey)
        return await getAllApiKeysFromDB(workspaceId)
    } catch (error) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.updateApiKey - ${getErrorMessage(error)}`)
    }
}
...
const deleteApiKey = async (id: string, workspaceId?: string) => {
    try {
        ...
        if (!dbResponse) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `ApiKey ${id} not found`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.deleteApiKey - ${getErrorMessage(error)}`)
    }
}
...
const importKeys = async (body: any) => {
    try {
        ...
        if (splitDataURI[0] !== 'data:application/json;base64') {
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid dataURI`)
        }
        ...
        if (!Array.isArray(keys)) {
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, `Invalid format: Expected an array of API keys`)
        }
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (typeof key !== 'object' || key === null) {
                throw new InternalDtamindError(StatusCodes.BAD_REQUEST, `Invalid format: Key at index ${i} is not an object`)
            }
        ...
        if (body.importMode === 'errorIfExist') {
            ...
            for (const key of keys) {
                ...
                if (keyNameExists) {
                    throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Key with name ${key.keyName} already exists`)
                }
            }
        }
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.importKeys - ${getErrorMessage(error)}`)
    }
}
...
const verifyApiKey = async (paramApiKey: string): Promise<string> => {
    try {
        ...
        if (!apiKey) {
            throw new InternalDtamindError(StatusCodes.UNAUTHORIZED, `Unauthorized`)
        }
        return 'OK'
    } catch (error) {
        if (error instanceof InternalDtamindError && error.statusCode === StatusCodes.UNAUTHORIZED) {
            throw error
        } else {
            throw new InternalDtamindError(
