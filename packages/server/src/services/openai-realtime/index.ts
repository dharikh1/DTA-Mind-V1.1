...
const buildAndInitTool = async (chatflowid: string, _chatId?: string, _apiMessageId?: string) => {
    ...
    if (!toolAgentNode) {
        throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Agent with tools not found in chatflow ${chatflowid}`)
    }
    ...
    if (!nodeToExecute) {
        throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Node not found`)
    }
...
const executeAgentTool = async (
    ...
    try {
        ...
        if (!tool) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Tool ${toolName} not found`)
        }
