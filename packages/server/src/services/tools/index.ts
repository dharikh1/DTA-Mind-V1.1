...
import { DTAMIND_METRIC_COUNTERS, DTAMIND_COUNTER_STATUS } from '../../Interface.Metrics'
...
const createTool = async (requestBody: any, orgId: string): Promise<any> => {
    try {
        ...
        appServer.metricsProvider?.incrementCounter(DTAMIND_METRIC_COUNTERS.TOOL_CREATED, { status: DTAMIND_COUNTER_STATUS.SUCCESS })
        return dbResponse
    } catch (error) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.createTool - ${getErrorMessage(error)}`)
    }
}
...
const deleteTool = async (toolId: string): Promise<any> => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.deleteTool - ${getErrorMessage(error)}`)
    }
}
...
const getAllTools = async (workspaceId?: string, page: number = -1, limit: number = -1) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.getAllTools - ${getErrorMessage(error)}`)
    }
}
...
const getToolById = async (toolId: string): Promise<any> => {
    try {
        ...
        if (!dbResponse) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Tool ${toolId} not found`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.getToolById - ${getErrorMessage(error)}`)
    }
}
...
const updateTool = async (toolId: string, toolBody: any): Promise<any> => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.updateTool - ${getErrorMessage(error)}`)
    }
}
    ...
    try {
        for (const data of newTools) {
            if (data.id && !validate(data.id)) {
                throw new InternalDtamindError(StatusCodes.PRECONDITION_FAILED, `Error: importTools - invalid id!`)
            }
        }
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.importTools - ${getErrorMessage(error)}`)
    }
}
