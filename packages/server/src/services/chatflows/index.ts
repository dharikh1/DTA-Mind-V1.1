...
const getChatflowByApiKey = async (apiKeyId: string, keyonly?: unknown): Promise<any> => {
    try {
        ...
        if (dbResponse.length < 1) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Chatflow not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalDtamindError(
...
const getChatflowById = async (chatflowId: string): Promise<any> => {
    try {
        ...
        if (!dbResponse) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowId} not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalDtamindError(
    ...
    try {
        ...
        if (dbResponse.chatbotConfig || uploadsConfig) {
            try {
                ...
                throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error parsing Chatbot Config for Chatflow ${chatflowId}`)
            }
        }
        return 'OK'
    } catch (error) {
