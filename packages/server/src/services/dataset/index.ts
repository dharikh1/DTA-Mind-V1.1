...
const getDataset = async (id: string, page: number = -1, limit: number = -1) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: datasetService.getDataset - ${getErrorMessage(error)}`)
    }
}
    ...
    try {
        ...
        await appServer.AppDataSource.transaction(async (entityManager) => {
            ...
            for (const row of rows) {
                ...
                if (!item) throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Dataset Row ${row.id} not found`)
...
const createDataset = async (body: any) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: datasetService.createDataset - ${getErrorMessage(error)}`)
    }
}
...
const updateDataset = async (id: string, body: any) => {
    try {
        ...
        if (!dataset) throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Dataset ${id} not found`)
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: datasetService.updateDataset - ${getErrorMessage(error)}`)
    }
}
...
const deleteDataset = async (id: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: datasetService.deleteDataset - ${getErrorMessage(error)}`)
    }
}
...
const updateDatasetRow = async (id: string, body: any) => {
    try {
        ...
        if (!item) throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Dataset Row ${id} not found`)
