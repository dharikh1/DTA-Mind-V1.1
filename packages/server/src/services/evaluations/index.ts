...
import { EvaluationRunner, ICommonObject } from 'dtamind-components'
...
const runAgain = async (id: string, baseURL: string, orgId: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: EvalsService.runAgain - ${getErrorMessage(error)}`)
    }
}
...
const isOutdated = async (id: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: EvalsService.isOutdated - ${getErrorMessage(error)}`)
    }
}
...
const getEvaluation = async (id: string) => {
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: EvalsService.getEvaluation - ${getErrorMessage(error)}`)
    }
}
