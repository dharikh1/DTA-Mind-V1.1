...
export abstract class BaseCommand extends Command {
    static flags = {
        DTAMIND_FILE_SIZE_LIMIT: Flags.string(),
        PORT: Flags.string(),
        ...
        DTAMIND_SECRETKEY_OVERWRITE: Flags.string(),
    ...
    protected onTerminate() {
        return async () => {
            try {
                ...
                setTimeout(async () => {
                    logger.info('Dtamind was forced to shut down after 30 secs')
                    await this.failExit()
                }, 30000)

                await this.stopProcess()
            } catch (error) {
                logger.error('There was an error shutting down Dtamind...', error)
            }
        }
    }
    ...
    async init(): Promise<void> {
        ...
        if (flags.DTAMIND_FILE_SIZE_LIMIT) process.env.DTAMIND_FILE_SIZE_LIMIT = flags.DTAMIND_FILE_SIZE_LIMIT
        ...
        if (flags.DTAMIND_SECRETKEY_OVERWRITE) process.env.DTAMIND_SECRETKEY_OVERWRITE = flags.DTAMIND_SECRETKEY_OVERWRITE
