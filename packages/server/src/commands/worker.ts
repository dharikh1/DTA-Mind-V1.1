...
export default class Worker extends BaseCommand {
    ...
    async run(): Promise<void> {
        logger.info('Starting Dtamind Worker...')
    ...
    async stopProcess() {
        try {
            ...
            logger.info(`Shutting down Dtamind Prediction Worker ${this.predictionWorkerId}...`)
            await predictionWorker.close()

            const upsertWorker = queueManager.getQueue('upsert').getWorker()
            logger.info(`Shutting down Dtamind Upsertion Worker ${this.upsertionWorkerId}...`)
            await upsertWorker.close()
        } catch (error) {
            logger.error('There was an error shutting down Dtamind Worker...', error)
            await this.failExit()
        }
