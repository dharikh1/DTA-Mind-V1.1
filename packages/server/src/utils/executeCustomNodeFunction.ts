    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(componentNodes, 'customFunction')) {
            try {
                ...
                throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error running custom function: ${error}`)
            }
        } else {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Node customFunction not found`)
        }
    } catch (error) {
