...
export const executeUpsert = async ({
    ...
    if (vsNodesWithFileUpload.length > 1) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, 'Multiple vector store nodes with fileUpload enabled')
    ...
    if (vsNodes.length > 1 && !stopNodeId) {
        ...
        throw new InternalDtamindError(StatusCodes.NOT_FOUND, 'No vector node found')
    }
...
export const upsertVector = async (req: Request, isInternal: boolean = false) => {
    ...
    try {
        ...
        if (!chatflow) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowid} not found`)
        }
        ...
        if (!workspace) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Workspace ${chatflowWorkspaceId} not found`)
        }
        ...
        if (!org) {
            throw new InternalDtamindError(StatusCodes.NOT_FOUND, `Organization ${workspace.organizationId} not found`)
        }
