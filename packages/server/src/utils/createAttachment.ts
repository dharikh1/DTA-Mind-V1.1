...
export const createFileAttachment = async (req: Request) => {
    ...
    if (!chatflowid || !isValidUUID(chatflowid)) {
        throw new InternalDtamindError(StatusCodes.BAD_REQUEST, 'Invalid chatflowId format - must be a valid UUID')
    }
    if (isPathTraversal(chatflowid)) {
        throw new InternalDtamindError(StatusCodes.BAD_REQUEST, 'Invalid path characters detected')
    }
