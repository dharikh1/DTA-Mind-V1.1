...
export const buildAgentGraph = async ({
    ...
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error buildAgentGraph - ${getErrorMessage(e)}`)
    }
}
...
const compileMultiAgentsGraph = async (params: MultiAgentsGraphParams) => {
    ...
    for (const workerNode of workerNodes) {
        ...
        try {
            ...
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error initialize worker nodes - ${getErrorMessage(e)}`)
        }
    }
        ...
        try {
            ...
            if (supervisorResult.moderations && supervisorResult.moderations.length > 0) {
                try {
                    ...
                    throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, getErrorMessage(e))
                }
            }
            ...
            throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error initialize supervisor nodes - ${getErrorMessage(e)}`)
        }
    }
}
...
const compileSeqAgentsGraph = async (params: SeqAgentsGraphParams) => {
    ...
    if (!startAgentNodes.length) throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, 'Start node not found')
    if (startAgentNodes.length > 1)
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, 'Graph should have only one start node')
    ...
    if (!endAgentNodes.length && !loopNodes.length) {
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, 'Graph should have at least one End/Loop node')
    }
    /*** End of Validation ***/
    ...
    for (const agentNodeId of getSortedDepthNodes(depthQueue)) {
        ...
        if (eligibleSeqNodes.includes(agentNode.data.name)) {
            try {
                ...
                throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error initialize agent nodes - ${getErrorMessage(e)}`)
            }
        }
    }
    ...
    try {
        ...
        throw new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Error compile graph - ${getErrorMessage(e)}`)
    }
}
