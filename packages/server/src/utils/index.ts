...
export const getEndingNodes = (
    ...
    let error: InternalDtamindError | null = null
    for (const endingNode of endingNodes) {
        const endingNodeData = endingNode.data
        if (!endingNodeData) {
            error = new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node ${endingNode.id} data not found`)
        ...
        if (!isEndingNode) {
            if (
                ...
                error = new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node must be either a Chain or Agent or Engine`)
                continue
            }
        }
    ...
    if (endingNodes.length === 0 || error === null) {
        error = new InternalDtamindError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending nodes not found`)
    }
