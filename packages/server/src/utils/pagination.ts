...
export const getPageAndLimitParams = (req: Request): Pagination => {
    ...
    if (req.query.page) {
        ...
        if (page < 0) {
            throw new InternalDtamindError(StatusCodes.PRECONDITION_FAILED, `Error: page cannot be negative!`)
        }
    }
    if (req.query.limit) {
        ...
        if (limit < 0) {
            throw new InternalDtamindError(StatusCodes.PRECONDITION_FAILED, `Error: limit cannot be negative!`)
        }
    }
    return { page, limit }
}
