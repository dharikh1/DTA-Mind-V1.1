...
export class WorkspaceController {
    ...
    public async switchWorkspace(req: Request, res: Response, next: NextFunction) {
        if (!req.user) {
            return next(new InternalDtamindError(StatusCodes.UNAUTHORIZED, `Unauthorized: User not found`))
        }
        let queryRunner
        try {
            queryRunner = getRunningExpressApp().AppDataSource.createQueryRunner()
            ...
            if (!org) throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
            ...
            req.session.save((err) => {
                if (err) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, GeneralErrorMessage.UNHANDLED_EDGE_CASE)
            })
    ...
    public async delete(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if (!workspaceId) {
                throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.INVALID_WORKSPACE_ID)
            }
            const workspaceService = new WorkspaceService()
            await queryRunner.startTransaction()
    ...
    public async setSharedWorkspacesForItem(req: Request, res: Response, next: NextFunction) {
        try {
            if (!req.user) {
                throw new InternalDtamindError(StatusCodes.UNAUTHORIZED, `Unauthorized: User not found`)
            }
            if (typeof req.params === 'undefined' || !req.params.id) {
