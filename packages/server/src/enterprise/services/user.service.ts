...
export class UserService {
    ...
    public validateUserId(id: string | undefined) {
        if (isInvalidUUID(id)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_ID)
    }
    ...
    public validateUserName(name: string | undefined) {
        if (isInvalidName(name)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_NAME)
    }

    public validateUserEmail(email: string | undefined) {
        if (isInvalidEmail(email)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
    }
    ...
    public validateUserStatus(status: string | undefined) {
        if (status && !Object.values(UserStatus).includes(status as UserStatus))
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_STATUS)
    }
    ...
    public encryptUserCredential(credential: string | undefined) {
        if (!credential || isInvalidPassword(credential))
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, GeneralErrorMessage.INVALID_PASSWORD)
    ...
    public async createNewUser(data: Partial<User>, queryRunner: QueryRunner) {
        ...
        if (user) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.USER_EMAIL_ALREADY_EXISTS)
    ...
    public async updateUser(newUserData: Partial<User> & { password?: string }) {
        ...
        try {
            ...
            if (!oldUserData) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)

            if (newUserData.updatedBy) {
                const updateUserData = await this.readUserById(newUserData.updatedBy, queryRunner)
                if (!updateUserData) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            }
