...
export class AccountService {
    ...
    public async resendVerificationEmail({ email }: { email: string }) {
        ...
        try {
            ...
            if (!user) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            if (user && user.status === UserStatus.ACTIVE)
                throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_EMAIL_ALREADY_EXISTS)

            if (!user.email) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
    ...
    private async ensureOneOrganizationOnly(queryRunner: QueryRunner) {
        ...
        if (organizations.length > 0) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, 'You can only have one organization')
    }
    ...
    private async createRegisterAccount(data: AccountDTO, queryRunner: QueryRunner) {
        ...
        switch (platform) {
            ...
            case Platform.CLOUD: {
                ...
                if (!data.user.email) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
            ...
            case Platform.ENTERPRISE: {
                if (data.user.tempToken) {
                    ...
                    if (user.email !== data.user.email)
                        throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
                    ...
                    if (!organizationUser)
                        throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationUserErrorMessage.ORGANIZATION_USER_NOT_FOUND)
                    ...
                    if (!assignedOrganization)
                        throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
                    data.organization = assignedOrganization
                    const tokenExpiry = new Date(user.tokenExpiry!)
                    const today = new Date()
                    if (today > tokenExpiry) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.EXPIRED_TEMP_TOKEN)
                    data.user.tempToken = ''
                    data.user.tokenExpiry = null
                    data.user.name = name
                    data.user.status = UserStatus.ACTIVE
            ...
            default:
                throw new InternalDtamindError(StatusCodes.BAD_REQUEST, GeneralErrorMessage.UNHANDLED_EDGE_CASE)
        }
    ...
    private async saveInviteAccount(data: AccountDTO, currentUser?: Express.User) {
        ...
        try {
            ...
            if (!workspace) throw new InternalDtamindError(StatusCodes.NOT_FOUND, WorkspaceErrorMessage.WORKSPACE_NOT_FOUND)
            ...
            if (!role) throw new InternalDtamindError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
            data.role = role
            const user = await this.userService.readUserByEmail(data.user.email, queryRunner)
            if (!user) {
    ...
    public async login(data: AccountDTO) {
        ...
        try {
            if (!data.user.credential) {
                ...
                throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_CREDENTIAL)
            }
            const user = await this.userService.readUserByEmail(data.user.email, queryRunner)
            if (!user) {
                await auditService.recordLoginActivity(data.user.email || '', LoginActivityCode.UNKNOWN_USER, 'Login Failed')
                throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            }
            if (!user.credential) {
            ...
            if (!compareHash(data.user.credential, user.credential)) {
                await auditService.recordLoginActivity(user.email || '', LoginActivityCode.INCORRECT_CREDENTIAL, 'Login Failed')
                throw new InternalDtamindError(StatusCodes.UNAUTHORIZED, UserErrorMessage.INCORRECT_USER_EMAIL_OR_CREDENTIALS)
            }
            if (user.status === UserStatus.UNVERIFIED) {
                await auditService.recordLoginActivity(data.user.email || '', LoginActivityCode.REGISTRATION_PENDING, 'Login Failed')
                throw new InternalDtamindError(StatusCodes.UNAUTHORIZED, UserErrorMessage.USER_EMAIL_UNVERIFIED)
            }
            ...
            if (Array.isArray(wsUserOrUsers)) {
                if (wsUserOrUsers.length > 0) {
                    ...
                    throw new InternalDtamindError(StatusCodes.NOT_FOUND, WorkspaceUserErrorMessage.WORKSPACE_USER_NOT_FOUND)
                }
            }
    ...
    public async verify(data: AccountDTO) {
        ...
        try {
            await queryRunner.startTransaction()
            if (!data.user.tempToken) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_TEMP_TOKEN)
            const user = await this.userService.readUserByToken(data.user.tempToken, queryRunner)
            if (!user) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            data.user = user
            data.user.tempToken = ''
    ...
    public async resetPassword(data: AccountDTO) {
        ...
        try {
            ...
            if (user.tempToken !== data.user.tempToken)
                throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_TEMP_TOKEN)
            ...
            if (Math.abs(diff) > expiryInMins) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, UserErrorMessage.EXPIRED_TEMP_TOKEN)
