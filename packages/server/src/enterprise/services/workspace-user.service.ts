...
export class WorkspaceUserService {
    ...
    public validateWorkspaceUserStatus(status: string | undefined) {
        if (status && !Object.values(WorkspaceUserStatus).includes(status as WorkspaceUserStatus))
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceUserErrorMessage.INVALID_WORKSPACE_USER_SATUS)
    }

    public validateWorkspaceUserLastLogin(lastLogin: string | undefined) {
        if (isInvalidDateTime(lastLogin))
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceUserErrorMessage.INVALID_WORKSPACE_USER_LASTLOGIN)
    }
    ...
    public async readWorkspaceUserByOrganizationIdUserId(
        ...
        if (!organization) throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
        const ownerRole = await this.roleService.readGeneralRoleByName(GeneralRole.OWNER, queryRunner)
    ...
    public async createWorkspaceUser(data: Partial<WorkspaceUser>) {
        ...
        if (workspaceUser) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceUserErrorMessage.WORKSPACE_USER_ALREADY_EXISTS)
        ...
        if (!createdBy) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
    ...
    public async createWorkspace(data: Partial<Workspace>) {
        ...
        const ownerRole = await this.roleService.readGeneralRoleByName(GeneralRole.OWNER, queryRunner)
        if (!ownerRole) throw new InternalDtamindError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
        ...
        if (!orgAdmin) throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationUserErrorMessage.ORGANIZATION_USER_NOT_FOUND)
    ...
    public async updateWorkspaceUser(newWorkspaserUser: Partial<WorkspaceUser>, queryRunner: QueryRunner) {
        ...
        if (!workspaceUser) throw new InternalDtamindError(StatusCodes.NOT_FOUND, WorkspaceUserErrorMessage.WORKSPACE_USER_NOT_FOUND)
        if (newWorkspaserUser.roleId && workspaceUser.role) {
            ...
            if (role.organizationId !== workspaceUser.role.organizationId) {
                throw new InternalDtamindError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
            }
        ...
        if (!updatedBy) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
    ...
    public async deleteWorkspaceUser(workspaceId: string | undefined, userId: string | undefined) {
        ...
        try {
            ...
            if (role.name === GeneralRole.OWNER)
                throw new InternalDtamindError(StatusCodes.BAD_REQUEST, GeneralErrorMessage.NOT_ALLOWED_TO_DELETE_OWNER)
