...
export class OrganizationService {
    ...
    public validateOrganizationId(id: string | undefined) {
        if (isInvalidUUID(id)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, OrganizationErrorMessage.INVALID_ORGANIZATION_ID)
    }
    ...
    public validateOrganizationName(name: string | undefined, isRegister: boolean = false) {
        if (isInvalidName(name)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, OrganizationErrorMessage.INVALID_ORGANIZATION_NAME)
        if (!isRegister && name === OrganizationName.DEFAULT_ORGANIZATION) {
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, OrganizationErrorMessage.ORGANIZATION_RESERVERD_NAME)
        }
    }
    ...
    public async updateOrganization(newOrganizationData: Partial<Organization>) {
        ...
        if (!oldOrganizationData) throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
