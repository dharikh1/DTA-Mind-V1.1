...
export class WorkspaceService {
    ...
    public validateWorkspaceId(id: string | undefined) {
        if (isInvalidUUID(id)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.INVALID_WORKSPACE_ID)
    }
    ...
    public validateWorkspaceName(name: string | undefined, isRegister: boolean = false) {
        if (isInvalidName(name)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.INVALID_WORKSPACE_NAME)
        if (!isRegister && (name === WorkspaceName.DEFAULT_PERSONAL_WORKSPACE || name === WorkspaceName.DEFAULT_WORKSPACE)) {
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.WORKSPACE_RESERVERD_NAME)
        }
    }
    ...
    public async readWorkspaceByOrganizationId(organizationId: string | undefined, queryRunner: QueryRunner) {
        ...
        if (!rolePersonalWorkspace) throw new InternalDtamindError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
    ...
    public async updateWorkspace(newWorkspaceData: Partial<Workspace>) {
        ...
        if (!oldWorkspaceData) throw new InternalDtamindError(StatusCodes.NOT_FOUND, WorkspaceErrorMessage.WORKSPACE_NOT_FOUND)
