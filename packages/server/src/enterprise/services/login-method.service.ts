...
export class LoginMethodService {
    ...
    public validateLoginMethodId(id: string | undefined) {
        if (isInvalidUUID(id)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_ID)
    }
    ...
    public validateLoginMethodName(name: string | undefined) {
        if (isInvalidName(name)) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_NAME)
    }
    ...
    public validateLoginMethodStatus(status: string | undefined) {
        if (status && !Object.values(LoginMethodStatus).includes(status as LoginMethodStatus))
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_STATUS)
    }
    ...
    public async encryptLoginMethodConfig(config: string | undefined) {
        if (!config) throw new InternalDtamindError(StatusCodes.BAD_REQUEST, LoginMethodErrorMessage.INVALID_LOGIN_METHOD_STATUS)
    ...
    public async createOrUpdateConfig(body: any) {
        ...
        try {
            ...
            if (!createdOrUpdatedByUser) throw new InternalDtamindError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            const organization = await this.organizationService.readOrganizationById(organizationId, queryRunner)
            if (!organization) throw new InternalDtamindError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
    ...
    public async updateLoginMethod(newLoginMethod: Partial<LoginMethod>) {
        ...
        if (!oldLoginMethod) throw new InternalDtamindError(StatusCodes.NOT_FOUND, LoginMethodErrorMessage.LOGIN_METHOD_NOT_FOUND)
