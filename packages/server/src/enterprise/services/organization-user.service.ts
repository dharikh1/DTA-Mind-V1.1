...
export class OrganizationUserService {
    ...
    public validateOrganizationUserStatus(status: string | undefined) {
        if (status && !Object.values(OrganizationUserStatus).includes(status as OrganizationUserStatus))
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, OrganizationUserErrorMessage.INVALID_ORGANIZATION_USER_SATUS)
    }
    ...
    public async readOrganizationUserByWorkspaceIdUserId(
        ...
        const workspace = await queryRunner.manager
            ...
            .getOne()
        if (!workspace) throw new InternalDtamindError(StatusCodes.NOT_FOUND, WorkspaceUserErrorMessage.WORKSPACE_USER_NOT_FOUND)
    ...
    public async readOrgUsersCountByOrgId(organizationId: string): Promise<number> {
        try {
            ...
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, OrganizationUserErrorMessage.ORGANIZATION_USER_NOT_FOUND)
        }
    }
    ...
    public async createOrganizationUser(data: Partial<OrganizationUser>) {
        ...
        if (organizationUser)
            throw new InternalDtamindError(StatusCodes.BAD_REQUEST, OrganizationUserErrorMessage.ORGANIZATION_USER_ALREADY_EXISTS)
