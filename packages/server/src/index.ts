...
export class App {
    ...
    async config() {
        ...
        this.app.get('/api/v1/ip', (request, response) => {
            response.send({
                ip: request.ip,
                msg: 'Check returned IP address in the response. If it matches your current IP address ( which you can get by going to http://ip.nfriedly.com/ or https://api.ipify.org/ ), then the number of proxies is correct and the rate limiter should now work correctly. If not, increase the number of proxies by 1 and restart Cloud-Hosted Dtamind until the IP address matches your own. Visit https://docs.dtamindai.com/configuration/rate-limit#cloud-hosted-rate-limit-setup-guide for more information.'
            })
        })
        ...
        // In monorepo, packages are in packages/ directory, not node_modules
        const packagePath = path.resolve(__dirname, '..', '..', '..', 'packages', 'ui')
        const uiBuildPath = path.join(packagePath, 'dist')
        const uiHtmlPath = path.join(packagePath, 'dist', 'index.html')
    ...
    async stopApp() {
        try {
            ...
            logger.error(`❌[server]: Dtamind Server shut down error: ${e}`)
        }
    }
}
...
export async function start(): Promise<void> {
    ...
    server.listen(port, host, () => {
        logger.info(`⚡️ [server]: Dtamind Server is listening at ${host ? 'http://' + host : ''}:${port}`)
    })
}
