...
import { DTAMIND_CHATID, getBaseClasses, getCredentialData, getCredentialParam } from '../../../src/utils'
...
class Upstash_VectorStores implements INode {
            ...
            for (let i = 0; i < flattenDocs.length; i += 1) {
                if (flattenDocs[i] && flattenDocs[i].pageContent) {
                    if (isFileUploadEnabled && options.chatId) {
                        flattenDocs[i].metadata = { ...flattenDocs[i].metadata, [DTAMIND_CHATID]: options.chatId }
                    }
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (upstashMetadataFilter) obj.filter += ` OR ${DTAMIND_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${DTAMIND_CHATID}`
            else obj.filter = `${DTAMIND_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${DTAMIND_CHATID}`
        }
