...
const dtamind_API_KEY = process.env.dtamind_API_KEY;
...
if (!dtamind_API_KEY) {
  console.error('dtamind_API_KEY is not set in environment variables');
  process.exit(1);
}
...
const validateApiKey = (req, res, next) => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ') && authHeader.split(' ')[1] === dtamind_API_KEY) {
    return next();
  }
  ...
  try {
    ...
    if (req.query.chatId && req.query.fileName) {
      ...
      const response = await fetch(url, {
        method: req.method,
        headers: {
          Authorization: `Bearer ${dtamind_API_KEY}`,
        },
      });
