...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${dtamind_CHATID} == "${options.chatId}" OR NOT EXISTS(${dtamind_CHATID})`
            else milvusFilter = `${dtamind_CHATID} == "${options.chatId}" OR NOT EXISTS(${dtamind_CHATID})`
        }
